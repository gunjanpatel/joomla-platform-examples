...
{
	const CLI_NAME = 'Install Joomla! from CLI';
  const CLI_VERSION ='IJ 0.1 RC [FirstStep] - www.alikonweb.it';
  private $_time;
	/**
    ...
    echo(JApplicationCliInstaller::CLI_NAME)."\n"; 
    echo(JApplicationCliInstaller::CLI_VERSION)."\n"; 
    ...
    $a= new CliProgressBar();  
    $a->initPBar(6,13);   
    $count=1; 
    $a->advancePBar($count,'initial');   
    $count++; 
    $a->advancePBar($count,'init db');   
    ...
    $count++;  //3
    $a->advancePBar($count,'create tables');   
    ...
    $a->advancePBar($count,'setup db');   
      ...
      $count++;  //5
    $a->advancePBar($count,'sample data');   
    ...
    $count++;  //6
    $a->advancePBar($count,'install completed');   
		$this->out('Done');
