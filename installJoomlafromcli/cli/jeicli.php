...
class InstallCli extends JApplicationCli {
    ...
    var $a = null;
     var $b = null;
    const CLI_NAME = 'Install Joomla! Extensions from CLI';
    const CLI_VERSION ='IJE 0.1 RC [FirstStep] - www.alikonweb.it';
    private $_time;
    ...
    public function doExecute() {
        ...
        echo(InstallCli::CLI_NAME)."\n"; 
        echo(InstallCli::CLI_VERSION)."\n"; 
         ...
         switch ($args[1])
        {
        	  case  '-f': 
        	   if (@$args[2]) {
            	...
            	echo '[WARNING] Missed [extensionfile] Joomla Extension parameter'."\n" ;
            	$this->help(3);
              exit(2);	 
        	   }	
        	   break;
        	  case  '-u': 
        	   if (@$args[2]) {
            	...
            	echo '[WARNING] Missed [extensionurllist] Joomla Extension parameter'."\n" ;
            	$this->help(7);
              exit(2);	 
        	   }	
        	   break;
        	  case  '-m': 
        	   if (@$args[2]) {
        	   	 $this->updatefromfile($args[2]);
        	   } else {
            	echo '[WARNING] Missed [filelist] Joomla Extension parameter'."\n" ;
            	$this->help(11);
              exit(2);	 
        	   }	
        	   break;
        	  case  '-w': 
        	   if (@$args[2]) {
        	   	 $this->updatefromweb($args[2]);
        	   } else {
            	echo '[WARNING] Missed [extensionurl] Joomla Extension parameter'."\n" ;
            	$this->help(15);
              exit(2);	 
        	   }	
        	   break;   
    ...
    protected function updatefromfolder($p_file) {
        ...
        if (!is_file($tmp_dest . '/' . $p_file)) {
            $this->out('File '.$p_file.' not found');
            return false;
        }
    ...
    protected function updatefromfile($l_file) {
        //  installfromlist
        $data = JFile::read(JPATH_SITE . "/cli/".$l_file);
        $files = explode(";", $data);
        var_dump($files);
    ...
    protected function updatefromweb($resource) {
        //$input = JFactory::getApplication()->input;
        jimport('joomla.updater.update');
        $urls = array();
        
        $bartask = 4;
        $this->b = new CliProgressBar();
        $this->b->initPBar($bartask, 13);
        $this->status = 1;
        $this->b->advancePBar($this->status, 'from web');     
            // Did you give us a URL?	
            if (!$resource) {
                echo 'COM_INSTALLER_MSG_INSTALL_ENTER_A_URL';
                JLog::add($resource.' is not valid url.');
                return false;
            }          
            // Handle updater XML file case:
            if (preg_match('/\.xml\s*$/', $resource)) {             
                $update = new JUpdate;
                $update->loadFromXML($resource);
                $package_url = trim($update->get('downloadurl', false)->_data);               
                if ($package_url) {
            ...
            $this->status++;
            $this->b->advancePBar($this->status, 'download:' . $resource);           
            JLog::add($resource.' downloading.');
            // Download the package at the URL given
            $p_file = JInstallerHelper::downloadPackage($resource);
         //  echo "\n\n";
         //         var_dump($p_file);
            // Was the package downloaded?
            if (!$p_file) {
                JLog::add($resource.' is not valid download.');
                $this->b->finishPBar();
            } else {
                $config = JFactory::getConfig();
                $tmp_dest = $config->get('tmp_path');
                $this->status++;                
                $this->b->advancePBar($this->status, 'unpack:' . $resource);
                JLog::add('an '.' unpack.');
                // Unpack the downloaded package file
                $package = JInstallerHelper::unpack($tmp_dest . '/' . $p_file, true);
                //var_dump($package);
             ...
             //   ob_start();       
                try
                    ...
                    JLog::add($resource.' is not valid install.');
                  } else {
                    // Package installed sucessfully                    
                    $this->status++;                    
                    $this->b->advancePBar($this->status, 'installed:' . $installer->manifest->name);
                    JLog::add($resource.' installed.'.$installer->manifest->name);            
                    // JInstallerHelper::cleanupInstall($package['packagefile'], $package['extractdir']);  
		            	}
		            }
		            catch (Exception $ex)
		        ...
		        //    	ob_end_clean();
		            	// Display the error		            
		            	JLog::add($urls['name'][$e].$ex->getMessage().$installer->manifest->name);    
		              $this->b->advancePBar($this->status, ' not installed:' . $installer->manifest->name);   		            
		            }
            }
